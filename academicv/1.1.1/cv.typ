#import "utils.typ": convert-string-to-color, convert-string-to-length
...
#import "layouts/unnumbered-list.typ": layout-unnumbered-list
...
#let showrules(settings, doc) = {
  ...
  show heading.where(
    ...
    #set align(left)
    #set text(font: settings.font-heading, size: settings.fontsize-heading, weight: "semibold")
  ...
  if ((section-key in info) and (info.at(section-key) != none)) or layout == "header" {
    ...
    if layout == "header" {
      ...
      block[
        ...
        } else if layout == "unnumbered-list" {
          layout-unnumbered-list(info.at(section-key), isbreakable: isbreakable)
        } else {
